...
impl<'a> Checkbox<'a, &'a mut bool> {
    pub fn new(checked: &'a mut bool, atoms: impl IntoAtoms<'a>) -> Self {
        Self::new_ref(checked, atoms)
    }

    pub fn without_text(checked: &'a mut bool) -> Self {
        Self::without_text_ref(checked)
    }
}

impl<'a, T: DerefMut<Target = bool>> Checkbox<'a, T> {
    pub fn new_ref(checked: T, atoms: impl IntoAtoms<'a>) -> Self {
        Self {
            checked,
    ...
    pub fn without_text_ref(checked: T) -> Self {
        Self::new_ref(checked, ())
    }
...
impl<T: DerefMut<Target = bool>> Widget for Checkbox<'_, T> {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        checkbox_ui(
            ui,
            &mut checked as &mut dyn DerefMut<Target = bool>,
            atoms,
            indeterminate,
        )
    }
...
fn checkbox_ui(
    ui: &mut Ui,
    checked: &mut dyn DerefMut<Target = bool>,
    mut atoms: Atoms<'_>,
    indeterminate: bool,
) -> Response {
